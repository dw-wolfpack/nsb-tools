(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const inputs = { context: (form.querySelector("[name=context]")||{}).value||"" };
      const res = window.NSB_COLD_OUTREACH.generate(inputs, window.NSB_UTILS.hash(JSON.stringify(inputs) + Date.now()));
      const text = "PUNCHY:\n" + res.punchy + "\n\n---\n\nFORMAL:\n" + res.formal;
      out.textContent = text; out.dataset.raw = text;
      if (window.NSB_INCREMENT_GEN) window.NSB_INCREMENT_GEN();
      addRecent("cold-outreach-email"); window.nsbAnalytics.track("tool_generate", { tool: "cold-outreach-email" });
    }
    ...
    copyBtn && copyBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t && window.NSB_UTILS.copyToClipboard(t)) { window.NSB_TOAST.show("Copied"); window.nsbAnalytics.track("tool_copy", { tool: "cold-outreach-email" }); } });
    saveBtn && saveBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t) { const s = window.NSB_UTILS.storage.get("nsb_saved",[]); s.push({tool:"cold-outreach-email",text:t,at:new Date().toISOString()}); window.NSB_UTILS.storage.set("nsb_saved", s.slice(-20)); window.NSB_TOAST.show("Saved"); } });
