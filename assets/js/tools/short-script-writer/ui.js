(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const seed = window.NSB_UTILS.hash(topic + Date.now());
      const text = window.NSB_SHORT_SCRIPT.generate({ topic }, seed);
      out.textContent = text; out.dataset.raw = text;
      if (window.NSB_INCREMENT_GEN) window.NSB_INCREMENT_GEN();
      addRecent("short-script-writer"); window.nsbAnalytics.track("tool_generate", { tool: "short-script-writer" });
    }
    ...
    copyBtn && copyBtn.addEventListener("click", () => { const t = out.dataset.raw || out.textContent; if (t && window.NSB_UTILS.copyToClipboard(t)) { window.NSB_TOAST.show("Copied"); window.nsbAnalytics.track("tool_copy", { tool: "short-script-writer" }); } });
    saveBtn && saveBtn.addEventListener("click", () => { const t = out.dataset.raw || out.textContent; if (t) { const s = window.NSB_UTILS.storage.get("nsb_saved",[]); s.push({tool:"short-script-writer",text:t,at:new Date().toISOString()}); window.NSB_UTILS.storage.set("nsb_saved", s.slice(-20)); window.NSB_TOAST.show("Saved"); } });
