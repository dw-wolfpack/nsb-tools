(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const inputs = { topic: (form.querySelector("[name=topic]")||{}).value||"", platform: (form.querySelector("[name=platform]")||{}).value||"instagram" };
      const res = window.NSB_CAPTION.generate(inputs, window.NSB_UTILS.hash(JSON.stringify(inputs) + Date.now()));
      const text = "CAPTION:\n" + res.caption + "\n\nCOMMENT PROMPT:\n" + res.commentPrompt;
      out.textContent = text; out.dataset.raw = text;
      if (window.NSB_INCREMENT_GEN) window.NSB_INCREMENT_GEN();
      addRecent("caption-generator"); window.nsbAnalytics.track("tool_generate", { tool: "caption-generator" });
    }
    ...
    copyBtn && copyBtn.addEventListener("click", () => { const t = out.dataset.raw || out.textContent; if (t && window.NSB_UTILS.copyToClipboard(t)) { window.NSB_TOAST.show("Copied"); window.nsbAnalytics.track("tool_copy", { tool: "caption-generator" }); } });
    saveBtn && saveBtn.addEventListener("click", () => { const t = out.dataset.raw || out.textContent; if (t) { const s = window.NSB_UTILS.storage.get("nsb_saved",[]); s.push({tool:"caption-generator",text:t,at:new Date().toISOString()}); window.NSB_UTILS.storage.set("nsb_saved", s.slice(-20)); window.NSB_TOAST.show("Saved"); } });
