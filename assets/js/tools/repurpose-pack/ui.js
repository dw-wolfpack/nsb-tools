(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const res = window.NSB_REPURPOSE.generate(inputs, window.NSB_UTILS.hash(JSON.stringify(inputs) + Date.now()));
      let text = "HOOKS:\n" + res.hooks.join("\n") + "\n\nSCRIPT:\n" + res.script + "\n\nCAPTIONS:\n" + res.captions.join("\n") + "\n\nVISUAL IDEAS:\n" + res.visuals.join("\n");
      out.textContent = text; out.dataset.raw = text;
      if (window.NSB_INCREMENT_GEN) window.NSB_INCREMENT_GEN();
      addRecent("repurpose-pack"); window.nsbAnalytics.track("tool_generate", { tool: "repurpose-pack" });
    }
    ...
    copyBtn && copyBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t && window.NSB_UTILS.copyToClipboard(t)) { window.NSB_TOAST.show("Copied"); window.nsbAnalytics.track("tool_copy", { tool: "repurpose-pack" }); } });
    saveBtn && saveBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t) { const s = window.NSB_UTILS.storage.get("nsb_saved",[]); s.push({tool:"repurpose-pack",text:t,at:new Date().toISOString()}); window.NSB_UTILS.storage.set("nsb_saved", s.slice(-20)); window.NSB_TOAST.show("Saved"); } });
