(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const inputs = { topic: (form.querySelector("[name=topic]")||{}).value||"", styles: (form.querySelector("[name=styles]")||{}).value||"howto,list,contrarian,story" };
      const results = window.NSB_HEADLINE.generate(inputs, window.NSB_UTILS.hash(JSON.stringify(inputs) + Date.now()));
      const text = results.map(r => "[" + r.style + "] " + r.text).join("\n");
      out.textContent = text; out.dataset.raw = text;
      if (window.NSB_INCREMENT_GEN) window.NSB_INCREMENT_GEN();
      addRecent("title-headline-generator"); window.nsbAnalytics.track("tool_generate", { tool: "title-headline-generator" });
    }
    ...
    copyBtn && copyBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t && window.NSB_UTILS.copyToClipboard(t)) { window.NSB_TOAST.show("Copied"); window.nsbAnalytics.track("tool_copy", { tool: "title-headline-generator" }); } });
    saveBtn && saveBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t) { const s = window.NSB_UTILS.storage.get("nsb_saved",[]); s.push({tool:"title-headline-generator",text:t,at:new Date().toISOString()}); window.NSB_UTILS.storage.set("nsb_saved", s.slice(-20)); window.NSB_TOAST.show("Saved"); } });
