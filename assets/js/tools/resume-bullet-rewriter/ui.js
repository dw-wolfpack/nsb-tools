(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const inputs = { bullet: (form.querySelector("[name=bullet]")||{}).value||"", count: (form.querySelector("[name=count]")||{}).value||"3" };
      const results = window.NSB_RESUME_BULLET.generate(inputs, window.NSB_UTILS.hash(JSON.stringify(inputs) + Date.now()));
