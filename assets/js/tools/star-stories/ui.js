(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function run() {
      ...
      const inputs = { notes: (form.querySelector("[name=notes]")||{}).value||"", mode: (form.querySelector("[name=mode]")||{}).value||"concise" };
      const text = window.NSB_STAR.generate(inputs, window.NSB_UTILS.hash(JSON.stringify(inputs) + Date.now()));
      out.textContent = text; out.dataset.raw = text;
      if (window.NSB_INCREMENT_GEN) window.NSB_INCREMENT_GEN();
      addRecent("star-stories"); window.nsbAnalytics.track("tool_generate", { tool: "star-stories" });
    }
    ...
    copyBtn && copyBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t && window.NSB_UTILS.copyToClipboard(t)) { window.NSB_TOAST.show("Copied"); window.nsbAnalytics.track("tool_copy", { tool: "star-stories" }); } });
    saveBtn && saveBtn.addEventListener("click", () => { const t = out.dataset.raw; if (t) { const s = window.NSB_UTILS.storage.get("nsb_saved",[]); s.push({tool:"star-stories",text:t,at:new Date().toISOString()}); window.NSB_UTILS.storage.set("nsb_saved", s.slice(-20)); window.NSB_TOAST.show("Saved"); } });
